...
public:
    ...
    // Send a full path (vector of layers). Each layer is a vector<Point2D>.
    // Protocol (text):
    // BEGIN_PATH <num_points_total>\n
    // x y z\n
    // ...
    // END_PATH\n
    bool sendPath(const std::vector<std::vector<Point2D>>& layers) {
        std::lock_guard<std::mutex> lk(sockMutex);
        if (!connected || sockfd < 0) return false;
 
	size_t totalPts = 0;       // <-- KEEP THIS ONE (line 605)
	for (const auto &l : layers) totalPts += l.size();

	std::ostringstream header;
	header << "BEGIN_PATH " << totalPts << "\n";
	send(sockfd, header.str().c_str(), header.str().size(), 0);

	

	// ------------------------------------------------------
	// Stream each point with gradually increasing Z
	// ------------------------------------------------------
	double z = 0.0;
	double z_step = 0.005;  // layer rise per point

	for (const auto &layer : layers) {
	    for (const auto &p : layer) {
		std::ostringstream pointMsg;
		pointMsg << p.x << " " << p.y << " " << z << "\n";
		std::string msg = pointMsg.str();

		if (send(sockfd, msg.c_str(), msg.size(), 0) < 0) {
		    std::cerr << "[RendererClient] send() failed, disconnecting renderer\n";
		    close(sockfd);
		    sockfd = -1;
		    connected = false;
		    return false;
	...
	// ------------------------------------------------------
	// Send footer (END_PATH) after all points are streamed
	// ------------------------------------------------------
	std::string footer = "END_PATH\n";
	send(sockfd, footer.c_str(), footer.size(), 0);
...
//
std::vector<Point3D> generateSpiralPathFromCrossSection(
    const std::vector<Point2D>& crossSection,
    double heightPerRevolution,
    int numRevolutions,
    bool makeDome = false)
{
    std::vector<Point3D> spiral;
    if (crossSection.empty()) return spiral;
    ...
    for (int rev = 0; rev < numRevolutions; ++rev) {
        double scale = 1.0;
        if (makeDome) {
            double t = (double)rev / (double)numRevolutions;
            // smoothly shrink for dome: from 1.0 -> 0.0
            scale = 1.0 - t;
            if (scale < 0.0) scale = 0.0;
        }

        for (int i = 0; i < N; ++i) {
            double x = crossSection[i].x * scale;
            double y = crossSection[i].y * scale;
            double z = (rev * N + i) * dz_per_point;
            spiral.emplace_back(x, y, z);
        }
    }
    return spiral;
}
...
int main() {
    ...
    while (true) {
		...
		// (This generator builds an upward spiral for 3D printer simulation)
		std::vector<Point3D> spiral = generateSpiralPathFromCrossSection(baseCross, 0.01, 50, domePhaseActive);

		// For now, just send the base 2D path to renderer (3D will be visualized externally)
		std::vector<std::vector<Point2D>> layers2D = { baseCross };
		rendererClient.sendPath(layers2D);
                }
            }
